...
public class US22Tests {
  ...
  void setup(){
    ...
    //criacao de uma solicitacao com tema de aluno
    solicitacao1 = solicitacaoRepository.save(
      Solicitacao.builder()
        .aluno(aluno.getId())
        .professor(professor.getId())
        .tema(temaAluno.getId())
        .aceito(false)
        .build()
    );
  ...
  class SolicitacaoVerificaStatus{
    ...
    @Test
    @DisplayName("Quando aceitamos uma solicitacao")
    void quandoAceitaUmaSolicitacao() throws Exception {
      //Arrange
      
      //Act
      String responseJsonString = driver.perform(put(URI_PROFESSORES + "/" + professor.getId() + "/aceitarSolicitacao")
      ...
      //Assert
      assertEquals(resultado.getAceito(), true);
    }
  }
}
